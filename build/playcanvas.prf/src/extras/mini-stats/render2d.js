...
const fragmentShaderGLSL = `
	...
	void main (void) {
		...
		if (uv0.w < graphSample.r)
			graph = vec4(1.0, 0.412, 0.380, 1.0);
		else if (uv0.w < graphSample.g)
			graph = vec4(0.467, 0.867, 0.467, 1.0);
		else if (uv0.w < graphSample.b)
			graph = vec4(0.424, 0.627, 0.863, 1.0);
		else
...
const fragmentShaderWGSL = `
	...
	@fragment fn fragmentMain(input : FragmentInput) -> FragmentOutput {
		...
		var graph: vec4f;
		if (uv0.w < graphSample.r) {
			graph = vec4f(1.0, 0.412, 0.380, 1.0);
		} else if (uv0.w < graphSample.g) {
			graph = vec4f(0.467, 0.867, 0.467, 1.0);
		} else if (uv0.w < graphSample.b) {
			graph = vec4f(0.424, 0.627, 0.863, 1.0);
		} else {
